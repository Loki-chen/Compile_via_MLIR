...
#   nisl_llvm_configure_bundled() : Configures the bundled LLVM and
#     sub-projects from third_party.
#   nisl_llvm_configure_installed() : Configures from installed LLVM and
#     sub-projects.
#   nisl_add_llvm_external_project(...) : Adds an external LLVM project in
#     a similar fashion to LLVM_EXTERNAL_PROJECTS define (but in a way that
#     does not require a source build of LLVM).

macro(nisl_llvm_configure_bundled)
  message(STATUS "Adding bundled LLVM source dependency")
  nisl_llvm_set_bundled_cmake_options()
  ...
  set(LLVM_CMAKE_DIR "${NISL_BINARY_DIR}/llvm-project/lib/cmake/llvm")
  ...
  set(LLVM_INCLUDE_DIRS
    ${NISL_SOURCE_DIR}/third_party/llvm-project/llvm/include
    ${NISL_BINARY_DIR}/llvm-project/include
  )
  set(MLIR_INCLUDE_DIRS
    ${NISL_SOURCE_DIR}/third_party/llvm-project/mlir/include
    ${NISL_BINARY_DIR}/llvm-project/tools/mlir/include
  )
  set(LLD_INCLUDE_DIRS
    ${NISL_SOURCE_DIR}/third_party/llvm-project/lld/include
    ${NISL_BINARY_DIR}/llvm-project/tools/lld/include
  )

  set(LLVM_BINARY_DIR "${NISL_BINARY_DIR}/llvm-project")
  set(LLVM_TOOLS_BINARY_DIR "${LLVM_BINARY_DIR}/bin")
  set(LLVM_EXTERNAL_LIT "${NISL_SOURCE_DIR}/third_party/llvm-project/llvm/utils/lit/lit.py")
endmacro()



macro(nisl_llvm_configure_installed)
...
macro(nisl_llvm_set_bundled_cmake_options)
  # When enabling an NISL CPU backend, automatically enable these targets.
  set(NISL_DEFAULT_CPU_LLVM_TARGETS "X86;ARM;AArch64;RISCV"
  ...
  set(NISL_CLANG_TARGET)
  set(NISL_LLD_TARGET)

  # Unconditionally enable some other cheap LLVM tooling.
  set(NISL_LLVM_LINK_TARGET llvm-link)

  # Unconditionally enable mlir.
  list(APPEND LLVM_ENABLE_PROJECTS mlir)

  # Configure LLVM based on enabled NISL target backends.
  message(STATUS "NISL compiler target backends:")
  if(NISL_TARGET_BACKEND_CUDA)
    message(STATUS "  - cuda")
    list(APPEND LLVM_TARGETS_TO_BUILD NVPTX)
    set(NISL_CLANG_TARGET clang)
  endif()
  if(NISL_TARGET_BACKEND_LLVM_CPU)
    message(STATUS "  - llvm-cpu")
    list(APPEND LLVM_TARGETS_TO_BUILD "${NISL_DEFAULT_CPU_LLVM_TARGETS}")
    set(NISL_CLANG_TARGET clang)
    set(NISL_LLD_TARGET lld)
  endif()
  if(NISL_TARGET_BACKEND_LLVM_CPU_WASM)
    message(STATUS "  - llvm-cpu (wasm)")
    list(APPEND LLVM_TARGETS_TO_BUILD WebAssembly)
    set(NISL_CLANG_TARGET clang)
    set(NISL_LLD_TARGET lld)
  endif()
  if(NISL_TARGET_BACKEND_METAL_SPIRV)
    message(STATUS "  - metal-spirv")
  endif()
  if(NISL_TARGET_BACKEND_ROCM)
    message(STATUS "  - rocm")
    list(APPEND LLVM_TARGETS_TO_BUILD AMDGPU)
  endif()
  if(NISL_TARGET_BACKEND_VULKAN_SPIRV)
    message(STATUS "  - vulkan-spirv")
  endif()
  if(NISL_TARGET_BACKEND_VMVX)
    message(STATUS "  - vmvx")
  endif()
  if(NISL_TARGET_BACKEND_WEBGPU)
    message(STATUS "  - webgpu")
  endif()

  if(NISL_CLANG_TARGET)
...
# nisl_add_llvm_external_project(name location)
...
function(nisl_llvm_add_external_project name location)
...
# nisl_llvm_add_usage_requirements(llvm_library usage_library)
# Adds |usage_library| as a link dependency of |llvm_library| in a way that
# is legal regardless of whether imported or built.
function(nisl_llvm_add_usage_requirements llvm_library usage_library)
